...
public:
	...
	virtual void OnRecv(NetID netid, const MsgPacket* packet)
	{
		printf("gameway OnRecv from netid ===========%u\n",netid);
		if (netid < m_gateway->m_user_size)
		{
			m_gateway->OnRecvUserMsg(netid, data, length);
			m_gateway->m_user_list[netid].last_active_time = m_gateway->m_current_time;
		}
	}
...
void GatewayModule::OnRecvUserMsg(NetID netid, const MsgPacket* packet)
{
...
void GatewayModule::OnRecvLoginServerMsg(const MsgPacket* packet)
{
