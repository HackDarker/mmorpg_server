...
local function encode_message(CObj, message_type, t)
	...
	for k,v in pairs(t) do
		local func = type[k]
		print("encode_messageaaaaaaaaaaaaaaaaa",k,v,func)
		func(CObj, k , v)
	end
end
...
function encode( message, t , func , ...)
	print("encode11111111111111111111",message,type(t))
	local encoder = c._wmessage_new(P, message)
	print("aaaaaaaaaaaaaaaaaaaaaaaaaa")
	assert(encoder ,  message)
	print("bbbbbbbbbbbbbbbbbbbbbbbbbb",encode_type_cache[message])
	encode_message(encoder, message, t)
	print("encode22222222222222222222222222")
	if func then
...
function decode(typename, buffer, length)
	print("decode11111111111111111111",typename,buffer,length)
	local ret = {}
	local ok = c._decode(P, decode_message_cb , ret , typename, buffer, length)
	print("decode22222222222222222",typename,buffer,length)
	if ok then
