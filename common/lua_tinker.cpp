...
#include "../networklib/lua_netPacket.h"
...
{
	lua_pushboolean(L, *(long long*)lua_topointer(L, 1) == *(long long*)lua_topointer(L, 2));
	return 1;
}
...
{
	lua_pushboolean(L, *(unsigned long long*)lua_topointer(L, 1) == *(unsigned long long*)lua_topointer(L, 2));
	return 1;
}
...
int  lua_tinker::make_function_hanlde(lua_State* L, const char* name)
{
	...
	if (lua_isfunction(L, -1))
	{
		return luaL_ref(L, LUA_REGISTRYINDEX);//luaL_ref把栈顶的值取出，放到指定的tabel中，然后返回一个索引
	}
	lua_pop(L, 1);
	return LUA_REFNIL; //-1
}

int lua_tinker::add_ref_table(lua_State* L, lua_tinker::table t)
{
	lua_tinker::push<lua_tinker::table>(L, t);
	return luaL_ref(L, LUA_REGISTRYINDEX);
}

lua_tinker::table lua_tinker::get_ref_table(lua_State* L, int ref)
{
	if(ref != LUA_REFNIL) {
		lua_rawgeti(L, LUA_REGISTRYINDEX, ref);
		if(lua_istable(L, -1)) {
			return pop<lua_tinker::table>(L);
		} else {
			lua_pop(L, 1);
			print_error(L, "lua_tinker::get_ref_table() attempt to get registry '%d' (not a table)", ref);
		}
	}
	return lua_tinker::table();
}
...
int lua_tinker::meta_get(lua_State *L)
{
	...
	lua_pushvalue(L,2);
	lua_rawget(L,-2);//key pop ,top = t[key]
