...
namespace lua_tinker
{
	...
	int     make_function_hanlde(lua_State* L, const char* name);
	...
	template<typename RVal>
	RVal call(lua_State* L, int func_handle)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);

		lua_rawgeti(L, LUA_REGISTRYINDEX, func_handle);
		if(lua_isfunction(L,-1))
		{
			if(lua_pcall(L, 0, 1, errfunc) != 0)
			{
		...
		{
			print_error(L, "lua_tinker::call() attempt to call registry `%d' (not a function)", func_handle);
		}
	...
	template<typename RVal, typename T1>
	RVal call(lua_State* L, int func_handle, T1 arg1)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);
	...
	template<typename RVal, typename T1, typename T2>
	RVal call(lua_State* L, int func_handle, T1 arg1, T2 arg2)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);
	...
	template<typename RVal, typename T1, typename T2, typename T3>
	RVal call(lua_State* L, int func_handle, T1 arg1, T2 arg2, T3 arg3)
	{
	...
	template<typename RVal, typename T1, typename T2, typename T3, typename T4>
	RVal call(lua_State* L, int func_handle, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);
	...
	template<typename RVal, typename T1, typename T2, typename T3, typename T4, typename T5>
	RVal call(lua_State* L, int func_handle, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);
	...
	template<typename RVal, typename T1, typename T2, typename T3, typename T4, typename T5, typename T6>
	RVal call(lua_State* L, int func_handle, T1 arg1, T2 arg2, T3 arg3, T4 arg4, T5 arg5, T6 arg6)
	{
		lua_pushcclosure(L, on_error, 0);
		int errfunc = lua_gettop(L);
	...
	{ 
		push_meta(L, class_name<T>::name());//push table
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, name);//push string
			new(lua_newuserdata(L,sizeof(F))) F(func);//构造插入栈中的内存块,block = func push_userdata
			push_functor(L, func);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	...
	struct table_obj
	{
		...
		T get(int i)
		{
			...
			if (lua_isnil(m_L, -1) && is_ptr<T>::value){
				lua_remove(m_L, -1);
				return (T)NULL;
			}
			return pop<T>(m_L);
		...
		bool can_get_next(bool& isFirst)
		{
			if (validate()){
				if(isFirst) {
					isFirst = false;
		...
		template<typename T>
		T get_next_key() // first call get_next_value, then call get_next_key
		{
			return read<T>(m_L, -1);
		}
	...
	struct table
	{
		...
		bool can_get_next(bool& isFirst)
		{
			return m_obj->can_get_next(isFirst);
		}

		template<typename T>
		T get_next_value()
		{
			return m_obj->get_next_value<T>();
		}

		template<typename T>
		T get_next_key() // first call get_next_value, then call get_next_key
		{
			return m_obj->get_next_key<T>();
		}
